    ...
    const buttons = officers.map((officerObj, i) => {
        ...
        if (officers.length > 1) {
            return (
                ...
                <button
                    onClick={() => setOfficerInfo(i, 10000)}
                    className={btnClass}
                    key={JSON.stringify(officerObj._id)}
                ></button>
            );
        } else {
            // If there is only one officer, disable changing carousel.
            return <button className={btnClass} key={JSON.stringify(officerObj._id)}></button>;
        }
    });

    // Change carousel's shown officer to next/selected.
    const setOfficerInfo = (num: number, changeDelay: number) => {
        setOfficer({
            id: officers[num]._id,
        ...
        for (let i = 0; i < officers.length; i++) {
            if (officers[i]._id == officer.id) {
                ...
                if (i == officers.length - 1) {
                    setOfficerInfo(0, 4000);
                } else {
                    setOfficerInfo(i + 1, 4000);
                }
