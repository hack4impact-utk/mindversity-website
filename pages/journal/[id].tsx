...
export async function getServerSideProps(context: NextPageContext) {
    const post: JournalEntry = await getPostById(context.query.id);
    const related = await getEntriesByType(post.category);
    return {
        props: {
            post: post,
            relatedEntries: related.filter(
                (p: JournalEntry) => p.id != post.id
            ),
        },
    };
}
